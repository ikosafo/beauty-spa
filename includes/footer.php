...
foreach (['facebook', 'x', 'linkedin', 'instagram', 'working_hours'] as $field) {
    $contact_data[$field] = $contact_data[$field] ?? '';
}
...
<style>
    ...
    .widget-services.ttm-service-list > li { width: 48%; margin: 0; padding: 0; }
    .widget-services.ttm-service-list li a {
        color: #f7c08a; font-weight: 500; text-decoration: none;
        font-size: 14px; line-height: 1.2; display: block;
    }
    .widget-services.ttm-service-list li a:hover { text-decoration: underline; color: #fff; }
    .widget-services.ttm-service-list ul { list-style: none; padding-left: 10px; margin: 0; }
            ...
            <div class="row">
                <div class="col-lg-6 col-md-7 col-sm-9 m-auto text-center">
                    <div class="footer-logo">
                        <img id="footer-logo-img" class="img-center" src="/cms/<?php echo htmlspecialchars($logo_path); ?>" alt="Golden View Logo">
                    </div>
                    <h4 class="widget-text ttm-textcolor-white">Stay Connected with Golden View</h4>
                    <form id="subscribe-form" class="newsletter-form" method="post" action="/subscribe.php">
                        <div class="mailchimp-inputbox clearfix" id="subscribe-content">
                            <p><input type="email" name="email" placeholder="Your Email Address..." required></p>
                            <p>
                                <button class="submit ttm-btn ttm-btn-size-md ttm-btn-shape-rounded ttm-btn-bgcolor-skincolor ttm-textcolor-white"
                                        style="background: #D4A017;" type="submit">
                                    Subscribe Now!
                                </button>
                            </p>
                        </div>
                    ...
                    <div class="widget flicker_widget clearfix">
                        ...
                        <div class="textwidget widget-text">
                            <div class="featured-icon-box icon-align-before-content icon-ver_align-top style3">
                                <div class="featured-icon"><div class="ttm-icon ttm-icon_element-onlytxt ttm-icon_element-color-skincolor ttm-icon_element-size-sm"><i class="fa fa-map-marker"></i></div></div>
                                <div class="featured-content"><div class="featured-desc"><p><?php echo htmlspecialchars($contact_data['address']); ?></p></div></div>
                            </div>
                            <div class="featured-icon-box icon-align-before-content icon-ver_align-top style3">
                                <div class="featured-icon"><div class="ttm-icon ttm-icon_element-onlytxt ttm-icon_element-color-skincolor ttm-icon_element-size-sm"><i class="fa fa-envelope-o"></i></div></div>
                                <div class="featured-content"><div class="featured-desc"><p><a href="mailto:<?php echo htmlspecialchars($contact_data['email']); ?>"><?php echo htmlspecialchars($contact_data['email']); ?></a></p></div></div>
                            </div>
                            <div class="featured-icon-box icon-align-before-content icon-ver_align-top style3">
                                <div class="featured-icon"><div class="ttm-icon ttm-icon_element-onlytxt ttm-icon_element-color-skincolor ttm-icon_element-size-sm"><i class="fa fa-phone"></i></div></div>
                                <div class="featured-content"><div class="featured-desc"><p><?php echo htmlspecialchars($contact_data['phone1']); ?></p></div></div>
                            </div>
                            <div class="featured-icon-box icon-align-before-content icon-ver_align-top style3">
                                <div class="featured-icon"><div class="ttm-icon ttm-icon_element-onlytxt ttm-icon_element-color-skincolor ttm-icon_element-size-sm"><i class="fa fa-clock-o"></i></div></div>
                                <div class="featured-content"><div class="featured-desc"><p><?php echo htmlspecialchars($contact_data['working_hours']); ?></p></div></div>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="d-flex flex-row align-items-center justify-content-end social-icons">
                        <ul class="social-icons list-inline">
                            <?php if (!empty($contact_data['facebook'])): ?>
                                <li><a href="<?php echo htmlspecialchars($contact_data['facebook']); ?>" target="_blank"><i class="fa fa-facebook"></i></a></li>
                            <?php endif; ?>
                            <?php if (!empty($contact_data['x'])): ?>
                                <li><a href="<?php echo htmlspecialchars($contact_data['x']); ?>" target="_blank"><i class="fa fa-twitter"></i></a></li>
                            <?php endif; ?>
                            <?php if (!empty($contact_data['linkedin'])): ?>
                                <li><a href="<?php echo htmlspecialchars($contact_data['linkedin']); ?>" target="_blank"><i class="fa fa-linkedin"></i></a></li>
                            <?php endif; ?>
                            <?php if (!empty($contact_data['instagram'])): ?>
                                <li><a href="<?php echo htmlspecialchars($contact_data['instagram']); ?>" target="_blank"><i class="fa fa-instagram"></i></a></li>
                            <?php endif; ?>
                        </ul>
                    </div>
                </div>
...
$(document).ready(function () {
    $('#subscribe-form').on('submit', function (e) {
        ...
        if (!email || !/^\S+@\S+\.\S+$/.test(email)) {
            $msg.html('<div class="alert alert-danger">Please enter a valid email.</div>');
            return;
        }
        ...
        $.ajax({
            ...
            success: function (resp) {
                if (resp && typeof resp === 'object' && resp.message) {
                    ...
                    $msg.html('<div class="alert alert-' + alertType + '">' + resp.message + '</div>');
